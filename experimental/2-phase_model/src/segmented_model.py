...
def second_phase(df, X_val, y_val, selected_model_name, select=True):
    ...
    for i in range(len(segmented_dfs)):
        ...
        if select and (i==0 or i==1) and (selected_model_name=='2-cluster K-means' or selected_model_name=='2-cluster Decision Trees'):
            dump(lr_model, f"{selected_model_name}-segment{i+1}-logistic_model.pkl")
