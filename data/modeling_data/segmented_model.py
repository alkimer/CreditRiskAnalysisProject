...
def segmentation(km_clusters, dt_leafs, path_X_train, path_y_train):
    ...
    try:
        ...
        print("Error: Make sure about the paths of all CSV files (X_train.csv, X_val_p.csv, y_train.csv, y_val.csv).")
        exit()
...
def visualize(model, X_train, dataframe, n_segments):
    ...
    sns.countplot(x=model, data=X_train)    # 'kmeans_segment'  #'dt_segment'
