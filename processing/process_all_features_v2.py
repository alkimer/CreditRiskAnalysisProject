...
def process_features():
    ...
    X_train = pd.read_csv("data/data_splitted/X_train.csv")
    X_val = pd.read_csv("data/data_splitted/X_val.csv")
    y_train = pd.read_csv("data/data_splitted/y_train.csv").select_dtypes(include='number').iloc[:, 0].astype(int)
    y_val = pd.read_csv("data/data_splitted/y_val.csv").select_dtypes(include='number').iloc[:, 0].astype(int)
    ...
    os.makedirs("data/processed/v2/balanced", exist_ok=True)

    # Guardar data procesada
    X_train_final.to_csv("data/processed/v2/X_train_processed.csv", index=False)
    X_val_final.to_csv("data/processed/v2/X_val_processed.csv", index=False)
    y_train.to_csv("data/processed/v2/y_train.csv", index=False)
    y_val.to_csv("data/processed/v2/y_val.csv", index=False)

    # Aplicar SMOTE
    smote = SMOTE(random_state=42)
    X_train_final_bal, y_train_bal = smote.fit_resample(X_train_final, y_train)
    
    X_train_final_bal.to_csv("data/processed/v2/balanced/X_train_processed_bal.csv", index=False)
    y_train_bal.to_csv("data/processed/v2/balanced/y_train_bal.csv", index=False)
